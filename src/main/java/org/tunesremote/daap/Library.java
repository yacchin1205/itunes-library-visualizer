...
public class Library {
	 ...
	 * Logger
	 */
	private final static Logger logger = LoggerFactory.getLogger(Library.class);
	 ...
	 */
	public long readSearch(TagListener listener, String search, long start,
			long items) {
		long total = -1;
		try {
			...
			String query = String
					.format("%s/databases/%d/containers/%d/items?session-id=%s&meta=dmap.itemname,dmap.itemid,dmap.persistentid,daap.songartist,daap.songalbum,daap.songtime,daap.songuserrating,daap.songtracknumber&type=music&sort=name&include-sort-headers=1&query=(('com.apple.itunes.mediakind:1','com.apple.itunes.mediakind:4','com.apple.itunes.mediakind:8')+('dmap.itemname:*%s*','daap.songartist:*%s*','daap.songalbum:*%s*'))&index=%d-%d",
							...
							session.libraryId, session.sessionId,
							encodedSearch, encodedSearch, encodedSearch, start,
							items);
			byte[] raw = RequestHelper.request(query, false);
			Response resp = ResponseParser.performParse(raw, listener,
					MLIT_PATTERN);
			// apso or adbs
			...
			logger.warn("readSearch Exception:" + e.getMessage());
		}

		logger.debug(String.format(
				"readSearch() finished start=%d, items=%d, total=%d", start,
				items, total));
	...
	public void readArtists(TagListener listener) {
		// check if we have a local cache create a wrapping taglistener to
		// create
		// local cache
		try {
			logger.debug("readArtists() requesting...");
			...
			int hits = ResponseParser.performSearch(raw, listener,
					MLIT_PATTERN, true);
			logger.debug(String.format("readArtists() total=%d", hits));
			raw = null;

		} catch (Exception e) {
			logger.warn("readArtists Exception:" + e.getMessage());
		}
	}
	...
	public void readAlbums(TagListener listener, String artist) {
		...
		try {
			...
			logger.warn("readAlbums Exception:" + e.getMessage());
		}
	...
	public void readAlbums(TagListener listener) {
		try {
			...
			// parse list, passing off events in the process
			final int hits = ResponseParser.performSearch(raw, listener,
					MLIT_PATTERN, false);
			logger.info("readAlbums Total:" + hits);
		} catch (Exception e) {
			logger.warn("readAlbums Exception:" + e.getMessage());
		}
	}
		...
		try {
			String temp = String
					.format("%s/databases/%d/containers/%d/items?session-id=%s&meta=dmap.itemname,dmap.itemid,daap.songartist,daap.songalbum,daap.songalbum,daap.songtime,daap.songuserrating,daap.songtracknumber&type=music&sort=album&query='daap.songalbumid:%s'",
							session.getRequestBase(), session.databaseId,
							session.libraryId, session.sessionId, albumid);
			...
			logger.warn("readTracks Exception:" + e.getMessage());
		}
			...
			byte[] raw = RequestHelper
					.request(
							String.format(
									...
									session.getRequestBase(),
									session.databaseId, session.libraryId,
	...
	public void readPlaylist(String playlistid, TagListener listener) {
		...
		try {
			...
			logger.warn("readPlaylists Exception:" + e.getMessage());
		}
	}
	...
	public void readRadioPlaylist(String playlistid, TagListener listener) {
		logger.debug(" in readRadioPlaylist");
		try {
			...
			byte[] raw = RequestHelper
					.request(
							String.format(
									...
									session.getRequestBase(),
									session.radioDatabaseId, playlistid,
									session.sessionId), false);
			...
			logger.warn("readRadioPlaylist Exception:" + e.getMessage());
		}
	}
	...
	public void readCurrentSong(TagListener listener) {
		...
		try {
			String temp = String
					.format("%s/ctrl-int/1/playstatusupdate?revision-number=1&session-id=%s",
							session.getRequestBase(), session.sessionId);

			// Refactor response into one that looks like a normal items request
			// and trigger listener
			Response resp = RequestHelper.requestParsed(temp, false).getNested(
					"cmst");
			...
			logger.warn("readCurrentSong Exception:" + e.getMessage());
		}
	}
