...
import net.yzwlab.daap.impl.PairingListener;
...
public class PairingServer extends Thread {
	...
	protected static PairingListener paired;

	protected ServerSocket server;
	protected final Random random = new Random();

	public PairingServer(PairingListener paired) {
		PairingServer.paired = paired;
	}

	@Override
	public void destroy() {
		logger.debug("Destroying PairingServer " + PORT);
		try {
			if ((this.server != null) && (!this.server.isClosed())) {
				logger.debug("Destroying Socket " + PORT);
				this.server.close();
				this.server = null;
			}
			this.interrupt();
		} catch (IOException e) {
			logger.warn("Failed to destroy", e);
		}
	}
		...
		try {
			...
			while (this == thisThread) {
				...
				try {
					...
					logger.info("someone paired with me!");

					// trigger a handler that says weve been paired pass back
					// the
					// random code we generated
					paired.paired(this, new Paired(socket.getInetAddress(),
							niceCode));
					...
					logger.info("Closing Socket " + PORT);
					if ((this.server != null) && (!this.server.isClosed())) {
						this.server.close();
					}
				}
			...
			logger.info("PairingServer thread stopped...");
		} catch (Exception e) {
