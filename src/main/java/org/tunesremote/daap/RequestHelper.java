...
import net.yzwlab.daap.graphics.BitmapFactory;
...
public class RequestHelper {
	...
	public static byte[] requestSearch(Session session, String search,
		...
		return request(
				String.format(
						...
						session.musicId, session.sessionId, encodedSearch,
						encodedSearch, encodedSearch, start, end), false);
	}

	public static byte[] requestTracks(Session session, String albumid)
			throws Exception {
	...
	public static byte[] requestAlbums(Session session, int start, int end)
			throws Exception {
	...
	public static Response requestParsed(String url, boolean keepalive)
			throws Exception {
		logger.debug(url);
	...
	public static void attemptRequest(String url) {
		try {
			...
			logger.warn("attemptRequest:" + e.getMessage());
		}
	}
	 ...
	 *             if any error occurs
	 */
	public static byte[] request(String remoteUrl, boolean keepalive)
			throws Exception {
	...
	public static Bitmap requestThumbnail(Session session, int itemid)
			throws Exception {
	...
	public static Bitmap requestThumbnail(Session session, int itemid,
		...
		try {
			byte[] raw = request(
					String.format(
							"%s/databases/%d/items/%d/extra_data/artwork?session-id=%s&mw=55&mh=55%s",
							session.getRequestBase(), session.databaseId,
							itemid, session.sessionId, type), false);
