...
public class ResponseParser {
	...
	public static int performSearch(byte[] raw, TagListener listener,
			Pattern listenFor, boolean haltmlit) throws IOException {
		logger.debug("ResponseParser performSearch...");
		final DataInputStream stream = new DataInputStream(
				new ByteArrayInputStream(raw));
		final int hits = ResponseParser.search(stream, listener, listenFor,
				stream.available(), haltmlit);
		listener.searchDone();
		return hits;
	}

	public static Response performParse(byte[] raw, TagListener listener,
			Pattern listenFor) throws IOException {
		logger.debug("ResponseParser performParse...");
		final DataInputStream stream = new DataInputStream(
				new ByteArrayInputStream(raw));
		final Response resp = ResponseParser.parse(stream, listener, listenFor,
				stream.available());
		listener.searchDone();
		return resp;
	}
	...
	private static int search(DataInputStream raw, TagListener listener,
			Pattern listenFor, int handle, boolean haltmlit) throws IOException {
		int hits = 0;
		...
		while (handle > 0) {
			...
			// check if we need to handle mlit special-case where it doesnt
			// branch
			if (haltmlit && key.equals("mlit")) {
				...
				if (listenFor.matcher(key).matches()) {
					...
					listener.foundTag(key, ResponseParser.parse(raw, listener,
							listenFor, length));
					hits++;
				} else {
					// recurse searching for other branches
					hits += ResponseParser.search(raw, listener, listenFor,
							length, haltmlit);
				}
	...
	private static Response parse(DataInputStream raw, TagListener listener,
			Pattern listenFor, int handle) throws IOException {
		final Response resp = new Response();
		int progress = 0;
		...
		while (handle > 0) {
			...
			final String nicekey = resp.containsKey(key) ? String.format(
					"%s[%06d]", key, progress) : key;

			if (BRANCHES.matcher(key).matches()) {
				// recurse off to handle branches
				final Response branch = ResponseParser.parse(raw, listener,
						listenFor, length);
				...
				resp.put(nicekey,
						new BigInteger(1, ResponseParser.readRaw(raw, length)));
			} else {
	...
	private static byte[] readRaw(DataInputStream raw, int length)
			throws IOException {
	...
	private static String readString(DataInputStream raw, int length)
			throws IOException {
