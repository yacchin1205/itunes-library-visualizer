...
public class Session {
	...
	public Session(String host, String pairingGuid) throws Exception {
		this(host, pairingGuid, null);
	}

	public Session(String host, String pairingGuid, Integer port)
			throws Exception {
		...
		Response login = RequestHelper.requestParsed(
				String.format("%s/login?pairing-guid=0x%s",
						this.getRequestBase(), pairingGuid), false);
		...
		Response databases = RequestHelper.requestParsed(
				String.format("%s/databases?session-id=%s",
						this.getRequestBase(), this.sessionId), false);
		for (Response resp : databases.getNested("avdb").getNested("mlcl")
				.findArray("mlit")) {
			// Local DB - mdbk = 1?
			if (resp.getNumberLong("mdbk") == 1 || !resp.containsKey("mdbk")) {
				...
				logger.debug(String.format("found database-id=%s",
						this.databaseId));
				...
				logger.debug(String.format("found radio-database-id=%s",
						this.radioDatabaseId));
			} else {
				// Other DB (mdbk = 2 = shared db?)
				// We have found another database I've seen shared libraries
				// appear
				// here
				logger.debug("found other-database = " + resp.getString("minm"));
			}
		}
		...
		Response playlists = RequestHelper
				.requestParsed(
						String.format(
								...
								this.getRequestBase(), this.databaseId,
								this.sessionId), false);

		for (Response resp : playlists.getNested("aply").getNested("mlcl")
				.findArray("mlit")) {
			...
			if (name.equals("Music")) {
				...
				// playlists
				if (name.equals("Films") || name.equals("TV Programmes")
						|| name.equals("iTunes U"))
				// Ignore
				{
				} else if (resp.getNumberLong("abpl") == 1) {
					this.libraryId = resp.getNumberLong("miid");
				} else {
					logger.debug(String.format("found playlist=%s", name));
					this.playlists.add(new Playlist(resp.getNumberLong("miid"),
							name, resp.getNumberLong("mimc"), resp
									.getNumberHex("mper")));
				}
			}
		}
		logger.debug(String.format("found music-id=%s", this.musicId));
	}

	private Status createStatus(StatusListener handler) {
		final Status stat = new Status(this, handler);
		stat.fetchUpdate();
		return stat;
	}

	public Status singletonStatus(StatusListener handler) {
		if (singleton == null || singleton.destroyThread.get())
			singleton = this.createStatus(handler);
		return singleton;
	}

	public Status notificationStatus(StatusListener handler) {
	...
	public String getRequestBase() {
		...
		if (port != null) {
			return String.format("http://%s:%d", host, port.intValue());
		}
		return String.format("http://%s:49152", host);
	}
		...
		ThreadExecutor.runTask(new Runnable() {
			public void run() {
				try {
					...
					logger.error("Fire Action Exception:" + e.getMessage());
				}
			}
		});
	}
	 ...
	 */
	public void logout() {
		logger.warn(String.format("Logging Out session-id=%s", this.sessionId));
		this.fireAction(String.format("%s/logout?session-id=%s",
				this.getRequestBase(), this.sessionId), false);
	}
	...
	public void controlPause() {
		...
		this.fireAction(
				String.format("%s/ctrl-int/1/pause?session-id=%s",
	...
	public void controlPlay() {
		...
		this.fireAction(
				String.format("%s/ctrl-int/1/playpause?session-id=%s",
						this.getRequestBase(), this.sessionId), true);
	}
	...
	public void controlNext() {
		...
		this.fireAction(
				String.format("%s/ctrl-int/1/nextitem?session-id=%s",
						this.getRequestBase(), this.sessionId), true);
	}
	...
	public void controlPrev() {
		...
		this.fireAction(
				String.format("%s/ctrl-int/1/previtem?session-id=%s",
						this.getRequestBase(), this.sessionId), true);
	}
	...
	public void controlVolume(long volume) {
		...
		this.fireAction(String.format(
				"%s/ctrl-int/1/setproperty?dmcp.volume=%s&session-id=%s",
	...
	public void controlProgress(int progressSeconds) {
		...
		this.fireAction(String.format(
				"%s/ctrl-int/1/setproperty?dacp.playingtime=%d&session-id=%s",
				this.getRequestBase(), progressSeconds * 1000, this.sessionId),
				true);
	}
	...
	public void controlShuffle(int shuffleMode) {
		...
		this.fireAction(String.format(
				"%s/ctrl-int/1/setproperty?dacp.shufflestate=%d&session-id=%s",
	...
	public void controlRepeat(int repeatMode) {
		...
		// HTTP/1.1
		this.fireAction(String.format(
				"%s/ctrl-int/1/setproperty?dacp.repeatstate=%d&session-id=%s",
	...
	public void controlRating(final long rating, final long trackId) {
		this.fireAction(
				String.format(
						...
						this.getRequestBase(), rating, trackId, this.sessionId),
				false);
	}
	...
	public void controlClearCue() {
		this.fireAction(String.format(
				"%s/ctrl-int/1/cue?command=clear&session-id=%s",
				getRequestBase(), sessionId), false);
	}
				...
				try {
					...
					RequestHelper.attemptRequest(String
							.format("%s/ctrl-int/1/cue?command=play&query='daap.songalbumid:%s'&index=%d&sort=album&session-id=%s",
									getRequestBase(), albumId, tracknum,
									sessionId));

					notifyStatus();
				} catch (Exception e) {
					logger.warn("Session Exception:" + e.getMessage());
				}
			}
		});
	...
	public void controlQueueAlbum(final String albumId) {
		ThreadExecutor.runTask(new Runnable() {
			public void run() {
				try {
					RequestHelper.attemptRequest(String
							.format("%s/ctrl-int/1/cue?command=add&query='daap.songalbumid:%s'&session-id=%s",
									getRequestBase(), albumId, sessionId));
			...
			public void run() {
				try {
					RequestHelper.attemptRequest(String.format(
							...
							getRequestBase(), sessionId));
					RequestHelper.attemptRequest(String
							.format("%s/ctrl-int/1/cue?command=play&query='daap.songartist:%s'&index=%d&sort=album&session-id=%s",
									getRequestBase(), encodedArtist,
									encodedIndex, sessionId));
	...
	public void controlQueueArtist(String artist) {
		...
		ThreadExecutor.runTask(new Runnable() {
			public void run() {
				try {
					RequestHelper.attemptRequest(String
							.format("%s/ctrl-int/1/cue?command=add&query='daap.songartist:%s'&session-id=%s",
									getRequestBase(), encodedArtist, sessionId));
					notifyStatus();
				} catch (Exception e) {
					logger.warn("Session Exception:" + e.getMessage());
				}
	...
	public void controlQueueTrack(final String trackId) {
		ThreadExecutor.runTask(new Runnable() {
			public void run() {
				try {
					RequestHelper.attemptRequest(String
							.format("%s/ctrl-int/1/cue?command=add&query='dmap.itemid:%s'&session-id=%s",
									getRequestBase(), trackId, sessionId));
					notifyStatus();
				} catch (Exception e) {
					logger.warn("Session Exception:" + e.getMessage());
				}
			...
			public void run() {
				try {
					RequestHelper.attemptRequest(String.format(
							...
							getRequestBase(), sessionId));
					RequestHelper.attemptRequest(String
							.format("%s/ctrl-int/1/cue?command=play&query='dmap.itemid:%s'&session-id=%s",
									getRequestBase(), trackId, sessionId));
					notifyStatus();
				} catch (Exception e) {
					logger.warn("Session Exception:" + e.getMessage());
				...
				try {
					...
					RequestHelper.attemptRequest(String
							.format("%s/ctrl-int/1/cue?command=play&query=(('com.apple.itunes.mediakind:1','com.apple.itunes.mediakind:4','com.apple.itunes.mediakind:8')+('dmap.itemname:*%s*','daap.songartist:*%s*','daap.songalbum:*%s*'))&type=music&sort=name&index=%d&session-id=%s",
									getRequestBase(), encodedSearch,
									encodedSearch, encodedSearch, index,
									sessionId));
					notifyStatus();
				} catch (Exception e) {
					logger.warn("Session Exception:" + e.getMessage());
	...
	public void controlPlayPlaylist(final String playlistPersistentId,
				...
				try {
					RequestHelper.attemptRequest(String
							.format("%s/ctrl-int/1/playspec?database-spec='dmap.persistentid:0x%s'&container-spec='dmap.persistentid:0x%s'&container-item-spec='dmap.containeritemid:0x%s'&session-id=%s",
									getRequestBase(), databasePersistentId,
			...
			public void run() {
				try {
					RequestHelper.request(
							String.format(
									"%s/ctrl-int/1/cue?command=play&index=%d&sort=album&session-id=%s",
									getRequestBase(), tracknum, sessionId),
							false);
					// on iTunes this generates a 501 Not Implemented response
				} catch (Exception e) {
					if (albumid != null && albumid.length() > 0) {
						// Fall back to choosing from the current album if there
						// is
						// one
						RequestHelper.attemptRequest(String
								.format("%s/ctrl-int/1/cue?command=clear&session-id=%s",
										getRequestBase(), sessionId));
						RequestHelper.attemptRequest(String
								.format("%s/ctrl-int/1/cue?command=play&query='daap.songalbumid:%s'&index=%d&sort=album&session-id=%s",
										getRequestBase(), albumid, tracknum,
	...
	public void controlVisualiser(boolean enabled) {
		...
		this.fireAction(String.format(
				"%s/ctrl-int/1/setproperty?dacp.visualizer=%d&session-id=%s",
	...
	public void controlFullscreen(boolean enabled) {
		...
		this.fireAction(String.format(
				"%s/ctrl-int/1/setproperty?dacp.fullscreen=%d&session-id=%s",
				this.getRequestBase(), enabled ? 1 : 0, this.sessionId), true);
	}
	...
	public void listContentCodes() {
		try {
			Response contentcodes = RequestHelper.requestParsed(
					String.format("%s/content-codes?session-id=%s",
							this.getRequestBase(), this.sessionId), false);

			for (Response resp : contentcodes.getNested("mccr").findArray(
					"mdcl")) {
				logger.debug("\"" + resp.getString("mcnm") + "\", \""
						+ resp.getString("mcna") + "\", \""
				...
				Response playlists = RequestHelper
						.requestParsed(
								String.format(
										"%s/databases/%d/containers?session-id=%s&meta=dmap.itemname,dmap.itemcount,dmap.itemid,dmap.persistentid,daap.baseplaylist,com.apple.itunes.special-playlist,com.apple.itunes.smart-playlist,com.apple.itunes.saved-genius,dmap.parentcontainerid,dmap.editcommandssupported",
										this.getRequestBase(),
										this.radioDatabaseId, this.sessionId),
								false);

				for (Response resp : playlists.getNested("aply")
						.getNested("mlcl").findArray("mlit")) {
					String name = resp.getString("minm");
					logger.debug(String.format("found radio genre=%s", name));
					this.radioGenres.add(new Playlist(resp
							.getNumberLong("miid"), name, resp
							.getNumberLong("mimc"), resp.getNumberHex("mper")));
				}
			} catch (Exception e) {
				logger.warn("getRadioGenres Exception:" + e.getMessage());
			}
		}
		return radioGenres;
	}

	public void playSpec(final long databaseId, final long containerId,
			final long itemId) {
		// GET
				...
				try {
					RequestHelper.attemptRequest(String.format(
							"%s/ctrl-int/1/playspec?"
									...
									+ "&session-id=%s", getRequestBase(),
							databaseId, containerId, itemId, sessionId));
					notifyStatus();
				} catch (Exception e) {
					logger.warn("Session Exception:" + e.getMessage());
				}
