...
import net.yzwlab.daap.graphics.Bitmap;
...
public class Status {
	...
	private final static int MAX_FAILURES = 100;
	...
	protected int repeatStatus = REPEAT_OFF, shuffleStatus = SHUFFLE_OFF,
			playStatus = STATE_PAUSED;
	protected boolean visualizer = false, fullscreen = false,
			geniusSelectable = false;
	...
	private StatusListener update = null;
	private final AtomicInteger failures = new AtomicInteger(0);
	private long revision = 1;
	public static String lastActivity, lastPlaylistId,
			lastPlaylistPersistentId;
	 ...
	 * @param update
	 */
	public Status(Session session, StatusListener update) {
	...
	public void updateHandler(StatusListener handler) {
		this.update = handler;
	}
		...
		public void run() {
			while (true) {
				...
				while (playStatus == STATE_PLAYING) {
					// logger.debug( "thread entering playing loop");
					if (destroyThread.get())
						break;
					long anchor = System.currentTimeMillis();
					try {
						final int failedCount = failures.get();
						Thread.sleep(1000 * Math.max(failedCount, 0));
					} catch (InterruptedException e) {
						// someone jolted us awake during playing, which means
						// track
						// changed
						logger.debug("someone jolted us during STATE_PLAYING loop");
						continue;
					}
					...
					notifyStatusUpdated(StatusUpdate.UPDATE_PROGRESS);

					// trigger a forced update if we seem to gone past end of
					// song
					if (progressRemain <= 0) {
						logger.debug("suggesting that we fetch new song");
						fetchUpdate();
					}
				}
				...
				while (playStatus == STATE_PAUSED) {
					logger.debug("thread entering paused loop");
					try {
						Thread.sleep(10000);
					} catch (InterruptedException e) {
						// someone probably jolted us awake with a status update
						logger.debug("someone jolted us during STATE_PAUSED loop");
					}
				...
				try {
					...
					logger.debug("someone jolted us during OVERALL loop");
				}

				if (destroyThread.get())
					break;
			}
			logger.warn("Status Progress Thread Killed!");
		}
	});
		...
		public void run() {
			while (true) {
				try {
					...
					final int failedCount = failures.get();
					Thread.sleep(1000 * 20 * Math.max(failedCount, 0));
					if (destroyThread.get())
						break;
					...
					parseUpdate(RequestHelper.requestParsed(
							String.format(
									...
									session.getRequestBase(), revision,
									session.sessionId), true));
					failures.decrementAndGet();
				} catch (Exception e) {
					logger.debug(
							String.format(
									"Exception in keepalive thread, so killing try# %d",
									failures.get()), e);
					if (failures.incrementAndGet() > MAX_FAILURES)
						destroy();
				}
			}
			logger.warn("Status KeepAlive Thread Killed!");
		}
	});

	public void destroy() {
		// destroy our internal thread
		logger.warn("trying to destroy internal status thread");
	...
	public void fetchUpdate() {
		logger.debug("Fetching Update From Server...");
			...
			public void run() {
				try {
					...
					parseUpdate(RequestHelper.requestParsed(
							String.format(
									"%s/ctrl-int/1/playstatusupdate?revision-number=%d&session-id=%s",
									session.getRequestBase(), 1,
									session.sessionId), false));
					failures.decrementAndGet();
				} catch (Exception e) {
					logger.warn("Failed to fetchUpdate", e);
					if (failures != null
							&& failures.incrementAndGet() > MAX_FAILURES) {
						destroy();
					}
				}
			}
	...
	protected void parseUpdate(Response resp) throws Exception {
		...
		StatusUpdate updateType = StatusUpdate.UPDATE_PROGRESS;
		...
		if (playStatus != this.playStatus
			...
			updateType = StatusUpdate.UPDATE_STATE;
			...
			logger.debug("about to interrupt #1");
			this.progress.interrupt();
		}
		...
		if (trackId != this.trackId || containerItemId != this.containerItemId
			...
			updateType = StatusUpdate.UPDATE_TRACK;
			...
			// this
			// new one
			logger.debug("about to interrupt #2");
			this.progress.interrupt();
		}
		...
		notifyStatusUpdated(updateType);
	}
				...
				public void run() {
					try {
						...
						coverCache = RequestHelper.requestBitmap(String.format(
								"%s/ctrl-int/1/nowplayingartwork?mw="
										+ screenHeight + "&mh=" + screenHeight
										+ "&session-id=%s",
								session.getRequestBase(), session.sessionId));
					} catch (Exception e) {
						logger.error("Fetch Cover Exception:" + e.getMessage());
					}
					coverEmpty = (coverCache == null);
					notifyStatusUpdated(StatusUpdate.UPDATE_COVER);
				}
			});
		}
	}
				...
				try {
					Response resp = RequestHelper.requestParsed(
							String.format(
									...
									session.getRequestBase(), databaseId,
									session.sessionId, trackId), false);
					...
					if (update != null) {
						...
						rating = entry.getNested("mlcl").getNested("mlit")
								.getNumberLong("asur");
						notifyStatusUpdated(StatusUpdate.UPDATE_RATING);
					}

				} catch (Exception e) {
					logger.error("Fetch Rating Exception:" + e.getMessage());
				}
			}
		});
	}
	...
	public long getVolume() {
		try {
			...
			logger.error("Fetch Volume Exception:" + e.getMessage());
		}
		return -1;
	}
	...
	public List<Speaker> getSpeakers(final List<Speaker> speakers) {
		try {
			logger.debug("getSpeakers() requesting...");
			...
			// Master volume is required to compute the speakers' absolute
			// volume
			...
			for (Response mdcl : mdclArray) {
				...
				speaker.setId(id);
				logger.debug("Speaker = " + speaker.getName());
				...
				int absoluteVolume = isActive ? (int) masterVolume
						* relativeVolume / 100 : 0;
				speaker.setAbsoluteVolume(absoluteVolume);
				speakers.add(speaker);
			}

		} catch (Exception e) {
			logger.error("Could not get speakers: ", e);
			speakers.clear();
			Speaker speaker = new Speaker();
	...
	/**
	 * Sets (activates or deactivates) the speakers as defined in the given
	 * list.
	 * 
	 * @param speakers
	...
	public void setSpeakers(List<Speaker> speakers) {
		...
		try {
			logger.debug("setSpeakers() requesting...");
			...
			logger.error("Could not set speakers: ", e);
		}
	}
	...
	public void setSpeakerVolume(long speakerId, int newVolume,
		...
		try {
			 ...
			 * only activated speaker, it will be controlled via the master
			 * volume.
			 *************************************************************/
			if (newVolume > masterVolume || formerVolume == speakersMaxVolume) {
				if (newVolume < secondMaxVolume) {
					...
					// then go on by decreasing the relative volume of this
					// speaker
			...
			logger.error("Error when setting speaker volume: ", e);
		}
	}
	...
	private void setAbsoluteVolume(long speakerId, int absoluteVolume)
		...
		url = String.format(
				"%s/ctrl-int/1/setproperty?dmcp.volume=%d&include-speaker-id=%s"
						+ "&session-id=%s", session.getRequestBase(),
				absoluteVolume, speakerId, session.sessionId);
		RequestHelper.request(url, false);
	}
	...
	private void setRelativeVolume(long speakerId, int relativeVolume)
		...
		url = String.format(
				"%s/ctrl-int/1/setproperty?speaker-id=%s&dmcp.volume=%d"
						+ "&session-id=%s", session.getRequestBase(),
				speakerId, relativeVolume, session.sessionId);
		RequestHelper.request(url, false);
	}
	 ...
	 *            変更された状態。nullは不可。
	 */
	private void notifyStatusUpdated(StatusUpdate updated) {
		if (updated == null) {
		...
		update.statusUpdated(this, updated);
	}
}
